    ...
    internal class MapBuilder
    {
        ...
        internal static Expression CreateBuilderExpression(MapMode mapMode, MapInfo mapInfo, Dictionary<Type, GenericAssociation> genericTypeAssociation, Expression paramSource, Expression varResult, Expression paramIncludeChain, List<Tuple<Type, Type>> usedBuilders=null)
        {
            ...
            if (varResult == null)
            {
                ...
                Expression initExpr = ctor != null ? (Expression)Expression.New(targetType) 
                    : Expression.Call(Meta.Method(() => FormatterServices.GetUninitializedObject(null)), Expression.Constant(targetType))
                    .Convert(targetType);
            ...
            {
                var typeAction = typeof(Action<,>).MakeGenericType(beforeMap.Method.GetParameters().Select(p=>p.ParameterType).ToArray());
            ...
            foreach (var kpMember in mapInfo.Members)
            {
                ...
                if (mapMode != MapMode.All 
                    && (kpMember.Value.RetrievalMode==RetrievalMode.RetrievedWhenSpecified || (kpMember.Value.RetrievalMode==RetrievalMode.Default && (!targetMemberType.IsSimpleType()) && (targetMemberType.IsClass || targetMemberType.IsInterface))))
                {
